...
class TanFeedForward(nn.Module):
    def __init__(self, in_dim, out_dim, kernel_size, expand_ratio=0.5):
        """
        :param in_dim: the input dimension
        :param out_dim: the output dimension. The input and output dimension should be the same.
        ...
        layers = []
        # the first linear layer is replaced by convolution.
        layers.extend([
            nn.Conv2d(in_dim, hidden_dim, kernel_size, padding=1, stride=1, bias=False),
            nn.BatchNorm2d(hidden_dim),
            nn.ReLU(inplace=True)
            ])
        # the second linear layer is replaced by convolution.
        layers.extend([
            nn.Conv2d(hidden_dim, out_dim, kernel_size, padding=1, stride=1, bias=False),
            nn.BatchNorm2d(out_dim)
        ])
        #print('layers', layers)
        self.conv = nn.Sequential(*layers)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if DEBUG:
            print('    x (before rearrange)', x.size())
        x = rearrange(x, 'b (h w) c -> b c h w', h=H, w=W)
        if DEBUG:
...
class LocalConvolutionalVisionTransformer2(nn.Module):
    def __init__(self,
                 in_chans=3,
                 num_classes=1000,
                 act_layer=nn.GELU,
...
def get_cls_model(config, **kwargs):
    ...
    msvit = LocalConvolutionalVisionTransformer2(
        in_chans=3,
...
if __name__ == '__main__':
    # python3 lib/models/cls_local_cvt2.py --cfg experiments/cifar100/cvt/local-cvt2-13-224x224.yaml 
